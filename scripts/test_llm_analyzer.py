...
async def test_llm_analysis(sql_hash: Optional[str] = None) -> None:
    """
    测试LLM分析功能
    ...
    try:
        ...
        # 7. 调用Qwen API
        logger.info(f"调用Qwen API分析SQL模式: {sql_pattern['sql_hash'][:8]}...")
        ...
        # 9. 如果需要，更新数据库中的分析结果
        update_db = input("是否将分析结果更新到数据库? (y/n): ").strip().lower()
        if update_db == 'y':
            await llm_service.update_sql_pattern_analysis_result(
                sql_hash=sql_pattern['sql_hash'],
                status="SUCCESS",
                relations_json=relations_json
            )
            logger.info(f"成功更新SQL模式 {sql_pattern['sql_hash'][:8]}... 的分析结果")
...
async def main():
    ...
    # 执行测试
    await test_llm_analysis(args.sql_hash)
