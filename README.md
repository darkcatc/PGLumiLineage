...
**PGLumiLineage：借助大语言模型（LLM）与图数据库（Apache AGE），实现PostgreSQL自动化数据血缘分析及知识图谱构建。**
...
* **自动化日志采集**: 通过SSH或本地文件方式，安全、自动地获取并处理PostgreSQL的运行时日志（CSV格式）。
* **全面元数据收集**: 从源PostgreSQL数据库中提取详尽的技术元数据（包括库、模式、表、视图、列、函数及其定义）。
* **智能SQL范式化与模式聚合**:
